...
var dialogHtml =
'<div id="exportDialog">' +
' <h3>Export ' + type.charAt(0).toUpperCase() + type.slice(1) + ' Data</h3>' +
' <form>' +
' <label>From Date: <input type="date" id="exportFromDate" required></la‐
bel><br>' +
' <label>To Date: <input type="date" id="exportToDate" required></label><br>' +
' <label>Max Records: <input type="number" id="exportMaxRecords" value="10000"
min="1" max="100000"></label><br>' +
' <button type="submit" id="exportConfirm">Download CSV</button>' +
...
$('#exportConfirm').click(function(e) {
e.preventDefault();
var fromDate = $('#exportFromDate').val();
var toDate = $('#exportToDate').val();
var maxRecords = $('#exportMaxRecords').val();
if (!fromDate || !toDate) {
alert('Please select both from and to dates');
return;
}
var dateField = type === 'entries' ? 'dateString' : 'created_at';
var url = '/api/v1/' + type + '.csv?count=' + maxRecords +
'&find[' + dateField + '][$gte]=' + fromDate +
'&find[' + dateField + '][$lte]=' + toDate + 'T23:59:59';
...
$('#exportToDate').val(today.toISOString().split('T')[0]);
$('#exportFromDate').val(thirtyDaysAgo.toISOString().split('T')[0]);
};
